...
const App = () => {
  ...
  if (user && admin && user.role === 'admin') return <AdminPage onBack={() => setAdmin(false)} />
  return user ? (new URLSearchParams(window.location.search).has('oauth') ? <OAuthConsent user={user} /> : <GroupPage user={user} onAdmin={() => setAdmin(true)} onLogout={() => { setUser(null); setAdmin(false) }} />) : <AuthPage onAuthenticated={setUser} />
}
export default App
