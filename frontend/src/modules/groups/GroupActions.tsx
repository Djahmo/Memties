...
export const GroupActions = ({ group }: { group: Group }) => {
  ...
  return <section className={deleting ? 'w-full' : ''} aria-label={t('Delete group')}>
    <fieldset disabled={busy} className="space-y-3">
      ...
      </> : <button type="button" className="secondary text-errorink border-errorink/30 hover:bg-error" onClick={() => setDeleting(true)}><Trash2 size={18} aria-hidden="true" />{t('Delete group')}</button>}
