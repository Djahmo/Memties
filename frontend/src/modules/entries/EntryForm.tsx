  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      onSaved(await api<Entry>(existing ? `/entries/${existing.id}` : '/entries', { method: existing ? 'PATCH' : 'POST', body: {
        tagId: tag?.id ?? null, title: String(form.get('title')).trim(), body: String(form.get('body')),
        occurredAt: new Date(String(form.get('occurredAt'))).toISOString(), groupId: destination, personIds: selected.map(person => person.id),
        ...(!existing && addReminder ? { reminder: { title: String(form.get('reminderTitle')).trim(), dueAt: new Date(`${form.get('reminderDate')}T${form.get('reminderTime')}`).toISOString(), notifyByPush: form.get('reminderPush') === 'on', notifyByEmail: form.get('reminderEmail') === 'on', language: i18n.language.startsWith('fr') ? 'fr' : 'en' } } : {}),
      } }))
