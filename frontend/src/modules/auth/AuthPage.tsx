...
export const AuthPage = ({ onAuthenticated }: { onAuthenticated: (user: User) => void }) => {
  ...
  useEffect(() => {
    ...
    api<{ localEnabled: boolean; registrationEnabled: boolean; ldapEnabled: boolean; samlEnabled: boolean }>('/auth/config').then(value => { if (active) { setLocalEnabled(value.localEnabled); setRegistrationEnabled(value.registrationEnabled); setProviders(value) } }).catch(() => { if (active) setError('Unable to load account settings. Refresh to try again.') })
    return () => { active = false }
  }, [])
  ...
  return <main className="min-h-svh grid grid-rows-[auto_1fr] md:(grid-cols-2 grid-rows-1)"><div className="absolute top-5 right-5"><PreferencesButton /></div>
    <section className="bg-[#173e36] text-white p-5 md:p-16 flex flex-col justify-between gap-6 md:gap-14">
      ...
      <div className="hidden md:block max-w-lg"><p className="uppercase tracking-[.2em] text-xs text-[#bad1c8] mb-5">{t("Your relationship memory")}</p><h1 className="text-4xl md:text-5xl font-semibold leading-tight">{t("Remember the people.")}<br />{t("Keep the context.")}</h1><p className="mt-6 text-[#d4e2db] text-lg leading-relaxed">{t("A quiet place to organize the people, conversations, and follow-ups that matter.")}</p></div>
      <p className="hidden md:block text-sm text-[#bad1c8]">{t("Personal by default. Shared with intention.")}</p>
    </section>
    <section className="flex items-center justify-center p-6 md:p-12">
      <div className="w-full max-w-sm"><p className="eyebrow">{t("Welcome to Memties")}</p><h2 className="text-3xl font-semibold mt-3 mb-2">{register ? t("Create your account") : t("Welcome back")}</h2><p className="muted mb-8">{register ? t("Your private Personal space is created automatically.") : t("Sign in to your relationship workspace.")}</p>
        {error && <p className="error mb-4" role="alert">{t(error)}</p>}
        {localEnabled && <form onSubmit={submit}><fieldset disabled={busy} className="space-y-5">
        ...
        {providers.samlEnabled && <button disabled={busy} className={`${localEnabled ? 'secondary' : 'primary'} w-full mt-3`} onClick={async () => {
