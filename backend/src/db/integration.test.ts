...
test('MySQL: reminder permissions, MCP tokens, group moves and external identity linking', { skip: !process.env.TEST_DATABASE_URL }, async t => {
  ...
  await db.insert(groupMembers).values({ groupId: child.id, userId: bob.user.id, role: 'viewer' })
  await groupService.move(alice.user.id, child.id, vault.id, null, true)
  assert.equal((await groupService.list(alice.user.id)).find(group => group.id === nested.id)?.isPrivate, true)
  assert.equal((await groupService.list(bob.user.id)).some(group => group.id === child.id || group.id === nested.id), false)
  await assert.rejects(groupService.move(alice.user.id, child.id, null), { statusCode: 403 })
  await assert.rejects(groupService.move(bob.user.id, child.id, null, null, true), { statusCode: 404 })
  await groupService.move(alice.user.id, child.id, null, null, true)
  assert.equal((await groupService.list(alice.user.id)).find(group => group.id === nested.id)?.isPrivate, false)
...
test('MySQL: registration, group privacy, permissions and session revocation', { skip: !process.env.TEST_DATABASE_URL }, async t => {
  ...
  assert.equal((await groupService.list(alice.user.id)).find(group => group.id === team.id)?.color, '#e11d48')
  assert.equal((await app.inject({ method: 'PATCH', url: `/api/groups/${team.id}`, headers, cookies: { memties_session: alice.token }, payload: { name: 'Renamed', color: 'red' } })).statusCode, 400)
