...
test('MySQL: reminder permissions, MCP tokens, group moves and external identity linking', { skip: !process.env.TEST_DATABASE_URL }, async t => {
  ...
  assert.ok(catalog.result.tools?.some(tool => tool.name === 'set_entry_archived'))
  ...
  assert.equal((await tool(read.token, 'set_entry_archived', { id: secret.id, archived: true })).isError, true)
  assert.equal((await tool(bobToken.token, 'set_entry_archived', { id: secret.id, archived: true })).isError, true)
...
test('MySQL: recursive contacts, independent entry privacy, filtered links and authorized moves', { skip: !process.env.TEST_DATABASE_URL }, async t => {
  ...
  await assert.rejects(memories.setArchived(bob.user.id, shared.id, true), { statusCode: 403 })
  await assert.rejects(memories.setArchived(bob.user.id, privateEntry.id, true), { statusCode: 404 })
  assert.ok((await memories.setArchived(alice.user.id, shared.id, true)).archivedAt)
  assert.equal((await memories.list(bob.user.id, historyInput.parse({}))).items.some(entry => entry.id === shared.id), false)
  assert.deepEqual((await memories.list(bob.user.id, historyInput.parse({ archive: 'archived' }))).items.map(entry => entry.id), [shared.id])
  assert.equal((await memories.list(bob.user.id, historyInput.parse({ archive: 'all' }))).items.some(entry => entry.id === shared.id), true)
  await memories.setArchived(alice.user.id, privateEntry.id, true)
  assert.deepEqual((await memories.list(bob.user.id, historyInput.parse({ archive: 'archived' }))).items.map(entry => entry.id), [shared.id])
  assert.equal((await memories.setArchived(alice.user.id, shared.id, false)).archivedAt, null)
  await memories.setArchived(alice.user.id, privateEntry.id, false)
  assert.equal((await memories.list(bob.user.id, historyInput.parse({}))).items.some(entry => entry.id === shared.id), true)
