...
test('MySQL: reminder permissions, MCP tokens, group moves and external identity linking', { skip: !process.env.TEST_DATABASE_URL }, async t => {
  ...
  assert.ok(catalog.result.tools?.some(tool => tool.name === 'delete_entry'))
  assert.equal((await tool(read.token, 'delete_entry', { id: shared.id })).isError, true)
  assert.equal((await tool(bobToken.token, 'delete_entry', { id: shared.id })).isError, true)
  assert.equal((await tool(bobToken.token, 'delete_entry', { id: secret.id })).isError, true)
  assert.equal((await app.inject({ method: 'DELETE', url: `/api/entries/${shared.id}`, headers: { origin } })).statusCode, 401)
  assert.equal((await app.inject({ method: 'DELETE', url: `/api/entries/${shared.id}`, headers: { origin }, cookies: { memties_session: bob.token } })).statusCode, 403)
  assert.equal((await app.inject({ method: 'DELETE', url: `/api/entries/${secret.id}`, headers: { origin }, cookies: { memties_session: bob.token } })).statusCode, 404)
  ...
  const disposable = await memories.create(alice.user.id, input)
  const deleted = await app.inject({ method: 'DELETE', url: `/api/entries/${disposable.id}`, headers: { origin }, cookies: { memties_session: bob.token } })
  assert.equal(deleted.statusCode, 200, deleted.body)
  assert.deepEqual(deleted.json(), { success: true })
  await assert.rejects(memories.get(alice.user.id, disposable.id), { statusCode: 404 })
  assert.equal((await db.select().from(reminders).where(eq(reminders.entryId, disposable.id))).length, 0)
  assert.ok(await contacts.get(alice.user.id, person.id))
  const mcpDisposable = await memories.create(alice.user.id, input)
  assert.equal((await tool(write.token, 'delete_entry', { id: mcpDisposable.id })).isError, undefined)
  await assert.rejects(memories.get(alice.user.id, mcpDisposable.id), { statusCode: 404 })
