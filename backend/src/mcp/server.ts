...
export const createMcpServer = (services: McpServices, identity: { userId: string; access: 'read' | 'write' }) => {
  ...
  server.registerTool('create_reminder', { description: 'Create a reminder on an existing writable entry. Enable email only if explicitly requested by the user.', inputSchema: wireReminder, annotations: write }, async input => run(true, () => services.reminders.create(userId, reminderInput.parse(input))))
