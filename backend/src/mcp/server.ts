...
export const createMcpServer = (services: McpServices, identity: { userId: string; access: 'read' | 'write' }) => {
  ...
  const wireHistory = listInput.extend({ archive: z.enum(['active', 'archived', 'all']).default('active'), personId: z.uuid().optional(), participantId: z.uuid().optional(), from: z.iso.datetime({ offset: true }).optional(), to: z.iso.datetime({ offset: true }).optional() })
  ...
  server.registerTool('set_entry_archived', { description: 'Archive or restore a writable entry. Archiving applies to everyone with access to the entry.', inputSchema: z.object({ id: z.uuid(), archived: z.boolean() }), annotations: { ...write, idempotentHint: true } }, async ({ id, archived }) => run(true, () => services.entries.setArchived(userId, id, archived)))
  ...
  server.registerTool('delete_entry', { description: 'Permanently delete an entry and all its reminders. Requires write access to the entry group. Linked contacts are preserved.', inputSchema: z.object({ id: z.uuid() }), annotations: { ...write, destructiveHint: true } }, async ({ id }) => run(true, () => services.entries.remove(userId, id)))
